...
describe 'Touchout without touching in' do
  it 'charges PENALTY_FARE' do
    ...
    card.top_up(10)
    expect { card.touch_out(station1) }.to change { card.balance }.by(-1 * Journey::PENALTY_FARE)
  end
end
...
describe 'Touching in twice without toching out in between' do
  it 'charges PENALTY_FARE' do
    ...
    card.top_up(10)
    card.touch_in(station1)
    expect { card.touch_in(station1) }.to change { card.balance }.by(-1 * Journey::PENALTY_FARE)
  end
end
...
describe 'Touching in and out' do
  it 'charges 1' do
    ...
    card.top_up(10)
    card.touch_in(station1)
    expect { card.touch_out(station2) }.to change { card.balance }.by(-1 * Journey::BASE_FARE - 1)
  end
end
